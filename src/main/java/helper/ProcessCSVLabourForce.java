 ...
 * 
 */
public class ProcessCSVLabourForce {

   // MODIFY these to load/store to/from the correct locations

   private static final String DATABASE = "jdbc:sqlite:database/ctg.db";
   private static final String CSV_FILE = "database/lga_labour_force_by_indigenous_status_by_sex_census_2016.csv";
      ...
      try {
         ...
         while (lineScanner.hasNext()) {
            ...
            while (rowScanner.hasNext() && indexCategory < category.length) {
               ...
               // demographic ID
               String query = "INSERT into Labour_Force (category, status, sex, lga, num_people) VALUES ("
         ...
         String idInsert = """
               ...
               FROM Demographic
               WHERE Demographic.sex_id = Labour_Force.sex AND
                     Demographic.indi_status = Labour_Force.status AND
                     Demographic.lga_code = Labour_Force.lga)
                  """;
         ...
         String columnDelete = """
               ALTER TABLE Labour_Force
               DROP COLUMN IF EXISTS sex, status, lga;
                     """;
         ...
         // Execute the statement, updating with matched demographic_id
         System.out.println("Executing: " + idInsert);
