 ...
 * 
 */
public class ProcessCSVNonSchool {

   // MODIFY these to load/store to/from the correct locations

   private static final String DATABASE = "jdbc:sqlite:database/ctg.db";
   private static final String CSV_FILE = "database/lga_non_school_education_by_indigenous_status_by_sex_census_2016.csv";
   ...
   public static void main(String[] args) {
      ...
      try {
         ...
         String initialise[] = {
               "DELETE FROM Non_School_Education;",
               "ALTER TABLE Non_School_Education ADD sex CHAR(1);",
               "ALTER TABLE Non_School_Education ADD status CHAR(1);",
               "ALTER TABLE Non_School_Education ADD lga INTEGER;"
         };
         ...
         while (lineScanner.hasNext()) {
            ...
            while (rowScanner.hasNext() && indexCategory < category.length) {
               ...
               // demographic ID
               String query = "INSERT into Non_School_Education (category, status, sex, lga, num_people) VALUES ("
         ...
         // and lga
         String idInsert = """
               UPDATE Non_School_Education
               SET id = (SELECT Demographic.demographic_id
               FROM Demographic
               WHERE Demographic.sex_id = Non_School_Education.sex AND
                     Demographic.indi_status = Non_School_Education.status AND
                     Demographic.lga_code = Non_School_Education.lga)
                  """;
         ...
         String cleanUp[] = {
               idInsert,
               "ALTER TABLE Non_School_Education DROP COLUMN sex;",
               "ALTER TABLE Non_School_Education DROP COLUMN status;",
               "ALTER TABLE Non_School_Education DROP COLUMN lga;"
         };
